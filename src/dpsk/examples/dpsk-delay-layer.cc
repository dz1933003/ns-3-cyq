...
public:
  ...
  void
  Setup (Ptr<DpskLayer> dpsk, uint32_t nPackets, uint32_t interval)
  {
    m_dpsk = dpsk;
    m_nPackets = nPackets;
...
class DpskDelayLayer : public DpskLayer
{
public:
  DpskDelayLayer () : m_delay (10), m_sendEvent ()
  {
  }

  virtual ~DpskDelayLayer ()
  {
  ...
  void
  Setup (uint32_t delay)
  {
    NS_ASSERT_MSG (delay > 0, "Delay must greater than zero");
    m_delay = delay;
  }

  virtual bool
  SendFromDevice (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol,
                  const Address &source, const Address &destination)
  {
    NS_LOG_FUNCTION (device << packet << protocol << &source << &destination);

    Time tNext (Seconds (m_delay));
    m_sendEvent = Simulator::Schedule (tNext, &DpskDelayLayer::DoSendFromDevice, this, device,
                                       packet, protocol, source, destination);
    return true;
  }

  void
  DoSendFromDevice (Ptr<NetDevice> device, Ptr<const Packet> packet, uint16_t protocol,
                    const Address &source, const Address &destination)
  {
    NS_LOG_UNCOND (Simulator::Now () << ": Delayed Send");
...
main (int argc, char *argv[])
{
  ...
  // Install Dpsk layer on Dpsk
  Ptr<DpskDelayLayer> senderLayer = CreateObject<DpskDelayLayer> ();
  senderLayer->InstallDpsk (senderDpsk);
  Ptr<DpskDelayLayer> recverLayer = CreateObject<DpskDelayLayer> ();
  recverLayer->InstallDpsk (recverDpsk);

  Ptr<DpskSendApp> senderApp = CreateObject<DpskSendApp> ();
  senderApp->Setup (senderLayer, 10, 1);
  ...
  recverApp->Setup (recverLayer);
  recverApp->SetStartTime (Seconds (10));
  recverApp->SetStopTime (Seconds (20));
