...
NS_LOG_COMPONENT_DEFINE ("NoPfcSwitchPort");

NS_OBJECT_ENSURE_REGISTERED (NoPfcSwitchPort);

TypeId
NoPfcSwitchPort::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::NoPfcSwitchPort")
                          .SetParent<Object> ()
                          .SetGroupName ("NoPfcSwitchPort")
                          .AddConstructor<NoPfcSwitchPort> ();
  return tid;
}

NoPfcSwitchPort::NoPfcSwitchPort ()
    : m_nQueues (0),
      m_lastQueueIdx (0),
      m_nInQueueBytes (0),
      m_nInQueuePackets (0),
...
void
NoPfcSwitchPort::SetupQueues (uint32_t n)
{
  NS_LOG_FUNCTION (n);
  CleanQueues ();
  m_nQueues = n;
...
void
NoPfcSwitchPort::SetDeviceDequeueHandler (DeviceDequeueNotifier h)
{
  m_mmuCallback = h;
}

Ptr<Packet>
NoPfcSwitchPort::Transmit ()
{
  NS_LOG_FUNCTION_NOARGS ();
...
bool
NoPfcSwitchPort::Send (Ptr<Packet> packet, const Address &source, const Address &dest,
...
bool
NoPfcSwitchPort::Receive (Ptr<Packet> p)
{
  NS_LOG_FUNCTION_NOARGS ();
...
Ptr<Packet>
NoPfcSwitchPort::DequeueRoundRobin (uint32_t &qIndex)
{
  NS_LOG_FUNCTION_NOARGS ();
...
Ptr<Packet>
NoPfcSwitchPort::Dequeue (uint32_t &qIndex)
{
  NS_LOG_FUNCTION_NOARGS ();
...
bool
NoPfcSwitchPort::CanDequeue (uint32_t qIndex)
{
